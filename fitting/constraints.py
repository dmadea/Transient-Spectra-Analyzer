...
class Constraint_Matrix(_ABC):
    """ Abstract class for constraints - only for all C or ST matrix
...
#!!!!!!   All concentraation matrix !!!!!!!!
class ConstraintClosure(Constraint_Matrix):
    """
    Closure constraint for all concentration matrix, either as constant sum at each data point (hard closure)
    or as scaling (smooth closure)
    ...
    def __init__(self, hard_closure=True, value=1, copy=False):
        self.copy = copy
        self.hard_closure = hard_closure
        self.value = value

    def transform(self, A, value=None):
        """ Apply closure constraint"""

        assert len(A.shape) == 2
        Closed = _np.empty_like(A)

        _value = value if value is not None else self.value

        if self.hard_closure:
            axis = 0 if A.shape[0] <= A.shape[1] else 1
            Closed = _value * A / A.sum(axis=axis, keepdims=True)
        # else:
...
class ConstraintNonpos(Constraint):
    """
    Non-positivity constraint. All positive entries made 0.
    ...
    def __init__(self, copy=False):
        """ A must be non-positive"""
        self.copy = copy

    def transform(self, A):
        """ Apply nonpositive constraint"""
        if self.copy:
            return A*(A < 0)
        else:
            A *= (A < 0)
