...
import lmfit
from lmfit import Parameters, fit_report, report_fit

q = np.random.rand(500, 100)


def chi2(params):
    x, y, z = params['x'].value, params['y'].value, params['z'].value

    return x * q + y * q * q + 5 * x + z * z + np.exp(-q)


params = Parameters()

params.add('x', 5)
params.add('y', -5, vary=True)
params.add('z', 0)
...
result = lmfit.minimizer.minimize(chi2, params, method='leastsq')
...
# def gpu_mul():
#
#     x_gpu = cp.random.rand(10000, 1000)
#     y_gpu = cp.random.rand(1000, 10000)
#
#     z = x_gpu.dot(y_gpu)
#
#     return cp.asnumpy(z)
...
#
# def cpu_mul():
#     x_cpu = np.random.rand(10000, 1000)
#     y_cpu = np.random.rand(1000, 10000)
#
#     z = x_cpu.dot(y_cpu)
#
#     return z
